...
with open(file_name_bus, 'r', encoding=enc) as file:
    ...
    ia = 0
    for line in reader:
        bus_coord[ia]    =    (float(line[2]), float(line[3]))
        ia+= 1
#	bus_coord.pop('ID')
...
for station, coord_list in metro_coord.items():
    ...
    for bus_id, bus_stop_coord in bus_coord.copy().items():
        if sqrt((coord_list[0][0] - bus_stop_coord[0])**2 + (coord_list[0][1] - bus_stop_coord[1])**2) < 0.012:
            c[bus_id]=bus_stop_coord
			
    for exit in coord_list:
        #print(station, exit)
        for bus_id, bus_stop_coord in c.copy().items():
